...
static u8 custom_edid3[] = {	// NSJ
// LG 4K monitor EDID
	0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x1e, 0x6d, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01,
	0x01, 0x18, 0x01, 0x03, 0x80, 0xa0, 0x5a, 0x78, 0x0a, 0xee, 0x91, 0xa3, 0x54, 0x4c, 0x99, 0x26,
	...
	
};
static u8 sel_edid;

int __init tegra_select_edid(char *options)
{
	if (!(strcmp(options, "apix540")))
		sel_edid = 1;
	else if (!(strcmp(options, "dell")))
		sel_edid = 2;
	else if (!(strcmp(options, "lg4k")))
		sel_edid = 3;
	else
		sel_edid = 0;
...
__setup("edid=",tegra_select_edid);
...
int tegra_edid_get_monspecs(struct tegra_edid *edid, struct fb_monspecs *specs)
{
	...
	switch(sel_edid)
	{
		case 1: custom_edid = custom_edid1; break;
		case 2: custom_edid = custom_edid2; break;
		case 3: custom_edid = custom_edid3; break;
		default:custom_edid = custom_edid1; break;
	}
 
	ret = tegra_edid_read_block(edid, 0, data);
	if (ret)	// NSJ
	{
		memcpy(data, custom_edid, 128);
		//goto fail;
	}
	...
	for (i = 1; i <= extension_blocks; i++) {
		...
		if (ret < 0)	// NSJ
		{
			memcpy(data + i * 128, custom_edid + i * 128, 128);
			//goto fail;
		}
...
fail:
	vfree(new_data);
	pr_info("NSJ tegra_edid_get_monspecs failed\n");
	return ret;
}
